...
fn parse_file(input: &str) -> IResult<&str, Entry> {
    let (input, (size, name)) =
        terminated(separated_pair(my_usize, char(' '), take_till(is_line_ending)),
                   line_ending)(input)?;
...
fn parse_folder(input: &str) -> IResult<&str, Entry> {
    ...
    let entry = Entry::Folder { name, entries: vec![] };
    Ok((input, entry))
}
...
fn parse_non_exiting_cd(input: &str) -> IResult<&str, Entry> {
    ...
    let entry = Entry::Folder { name: folder_name, entries };
    Ok((input, entry))
}
...
enum Entry<'a> {
    ...
    Folder { name: &'a str, entries: Vec<Entry<'a>> },
}
...
mod test {
    ...
    fn test_parse_file() {
        assert_eq!(
            ...
            Ok(("", Entry::File { name: "b.txt", size: 14848514 }))
        );
    }
    ...
    fn test_parse_folder() {
        assert_eq!(
            parse_folder("dir a\n"),
            Ok(("", Entry::Folder { name: "a", entries: vec![] }))
        );
    }
    ...
    fn test_parse_entry() {
        assert_eq!(
            ...
            Ok(("", Entry::File { name: "asdf", size: 678912345 }))
        );

        assert_eq!(
            parse_folder("dir f\n"),
            Ok(("", Entry::Folder { name: "f", entries: vec![] }))
        );
    }
    ...
    fn test_parse_ls() {
        assert_eq!(
            parse_ls("$ ls\n4060174 j\n8033020 d.log\n5626152 d.ext\n7214296 k\n"),
            Ok(("", vec![
                Entry::File { name: "j", size: 4060174 },
                Entry::File { name: "d.log", size: 8033020 },
                Entry::File { name: "d.ext", size: 5626152 },
                Entry::File { name: "k", size: 7214296 },
            ]))
        );
    }
    ...
    fn test_parse_non_exiting_cd() {
        ...
        assert_eq!(
            ...
            Ok(("", Entry::Folder {
                name: "d",
                entries: vec![
                    Entry::File { name: "b", size: 4060174 },
                    Entry::File { name: "d.log", size: 8033020 },
                    Entry::File { name: "d.ext", size: 5626152 },
                    Entry::File { name: "k", size: 7214296 },
                ],
            ...
            Ok(("", Entry::Folder {
                ...
                entries: vec![
                    Entry::Folder {
                        name: "a",
                        entries: vec![
                            Entry::File { name: "b.txt", size: 123 }
                        ],
                    }
                ],
            }))
    ...
    fn test_parse_exiting_cd() {
        ...
        assert_eq!(
            ...
            Ok(("", Entry::Folder {
                name: "d",
                entries: vec![
                    Entry::File { name: "a", size: 4060174 },
                    Entry::File { name: "d.log", size: 8033020 },
                    Entry::File { name: "d.ext", size: 5626152 },
                    Entry::File { name: "k", size: 7214296 },
                ],
